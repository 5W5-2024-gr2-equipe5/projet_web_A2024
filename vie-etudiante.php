<?php get_header() ?>
    <h1>Vie étudiante</h1>
    <div id="comite">
      <h2>Comité étudiant</h2>
      <p>
        Lorem ipsum dolor sit, amet consectetur adipisicing elit. Voluptas
        dolore sit eius cum ea. Incidunt officia doloribus optio error,
        repudiandae quos ex fuga consequuntur assumenda ea deserunt quas, itaque
        impedit.
      </p>
    </div>
    <div id="event">
      <h2>Évènements</h2>
      <p>
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Tenetur ratione
        modi voluptatem, quaerat commodi temporibus vel magni natus dolore,
        minima laudantium reiciendis at earum ea deleniti porro vero architecto
        facilis?
      </p>
    </div>
    <div id="bourses">
      <h2>Bourses</h2>
      <h3>Bourse Perspective</h3>
      <p>
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Quod blanditiis
        voluptates iusto sunt omnis culpa ea, eligendi ad esse nihil consequatur
        nobis architecto ex facere atque eveniet incidunt perspiciatis velit!
      </p>

      <h3>Aide financière aux études (AFE)</h3>
      <p>
        Lorem ipsum dolor sit amet, consectetur adipisicing elit. Sed quis
        repellendus voluptatem dolorum hic eligendi amet eum facilis commodi eos
        explicabo, alias dolores obcaecati. Dolorem natus iusto beatae
        repellendus asperiores!
      </p>
    </div>
    <?php get_footer() ?>